...
RSpec.describe TodoList, type: :model do
  context 'validations test' do 
    ...
    it 'ensures insert is correctly' do 
      todo_list = TodoList.new(title: 'testing', description: 'testing', user_id: 1, done_homework: true).save
      expect(todo_list).to eq(true)
    end  

    it 'ensures that is saving relation' do 
  ...
  context 'ensures TodoList is saving' do 
    it 'ensures that show is working' do 
      todo_list_show = TodoList.find(1) 
      expect(todo_list_show.id).to eq(1)
    end
  end

  context 'ensures TodoList is saving' do 
    it 'ensures todo lists is saving' do 
      todo_list_save = TodoList.new(title: 'test', description: 'test description', done_homework: 1, user_id: 2).save
  ...
  context 'review update TodoList' do 
    it 'ensures that the TodoList is Updating content' do
      todo_list_update = TodoList.find(3).update(title: 'test', description: 'test description', done_homework: 1, user_id: 2)
